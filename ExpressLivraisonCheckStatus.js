function trackExpressLivraisonParcelsFromSheet() {

  const TOKEN_EXPRESSLIVRAISON = 'EXPRESS_TOKEN';
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (carrier !== "Expresslivraison" || synced !== "Synced" || !parcelCode) {
      continue;
    }

    const url = `https://expresslivraison.ma/apiclient/trackparcel?parcel_code=${encodeURIComponent(parcelCode)}`;

    const options = {
      method: "get",
      headers: {
        Authorization: TOKEN_EXPRESSLIVRAISON,
    ...
    try {
      ...
      // ✅ Extract LAST delivery step description
      let deliveryStatus = "";
      if (
        json.tracking.history &&
        Array.isArray(json.tracking.history) &&
        json.tracking.history.length > 0
      ) {
        const lastStep = json.tracking.history[json.tracking.history.length - 1];
        deliveryStatus = lastStep.description || "";
      }

      const deliveryStatusCode = json.tracking.current_status || "";
  ...
  Logger.log("🏁 Tracking completed for Expresslivraison");
}
